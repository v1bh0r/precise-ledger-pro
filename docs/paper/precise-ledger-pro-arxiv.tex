...
\begin{itemize}
  ...
  \item Every adjustment entry carries full source-activity attribution,
        satisfying R3.
  \item The algorithm runs in $O(m)$ time over post-fork source activities,
        where $m$ is the number of distinct replay activities
        (Section~\ref{sec:alg-backdated}).
\end{itemize}
...
$O(m)$ entries per retroactive event (where $m$ is the total number of
replay activities after the fork point, as defined in
Section~\ref{sec:alg-backdated}). With suppression, only the $k$
