...
class "Ledger" as ledger {
  ...
  + synchronizeHistoricalTransactions(retroactiveLedger: Ledger): void
  - clone(): Ledger
}
...
note right of ledger::synchronizeHistoricalTransactions
...
:Get un-applied ledger activities;
...
repeat
  :Get the next un-applied ledger activity;
      if (The ledger activity is back dated) then (out of order activity)
        ...
        :ledger.synchronizeHistoricalTransactions(retroactiveLedger);
        :Mark the ledger activity as applied;
        ...
        'rollback k baad recursion hogi. We need to re-apply all the transactions that came after the reversal
        :ledger.synchronizeHistoricalTransactions(retroactiveLedger);
